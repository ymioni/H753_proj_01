...
static	tCb_GetData_LSM6DSO		Main_Cb_GetData_LSM6DSO;
...
static	tCmd_LSM6DSO			Main_Cmd			= 0;
...
/* USER CODE BEGIN PFP */
static	bool			BSP_LSM6DSO_Enqueue( tCmd_LSM6DSO Cmd, bool Set);
static	tCmdQueue		BSP_LSM6DSO_Dequeue( void);
static	bool			BSP_LSM6DSO_Transaction( tCmdQueue Rec);
static	bool			BSP_LSM6DSO_Transaction_TxRx(void);
static	bool			BSP_LSM6DSO_Transaction_Tx(void);
static	bool			BSP_LSM6DSO_Transaction_Rx(void);
static	bool			BSP_LSM6DSO_Transaction_SetData(tCmd_LSM6DSO Cmd);
static	void			BSP_LSM6DSO_Cb_TxDone(bool result);
static	void			BSP_LSM6DSO_Cb_RxDone(bool result);
...
bool			BSP_LSM6DSO_Init( I2C_HandleTypeDef *handle, tCb_GetData_LSM6DSO	CbFunc)
{
	...
	Main_Handle					= handle;
	Main_Cb_GetData_LSM6DSO		= CbFunc;

	Main_Per_DataResp.Address	= (I2C_DEVICE_ADDRESS_LSM6DSO >> 1);
	...
	BSP_LSM6DSO_Cmd( &Main_Per_DataCmd);
...
void 			BSP_LSM6DSO_MainLoop( void)
{
	...
	case	0:
		...
		tCmdQueue Rec = BSP_LSM6DSO_Dequeue();
		if( Rec.Cmd != 0)
		{
			BSP_LSM6DSO_Transaction(Rec);
			Main_State ++;
		}
		break;
...
bool			BSP_LSM6DSO_Cmd( tBSP_PER_DataCmd	*cmd)
{
	...
	case	eBSP_PER_FUNC_GET_SN:
		result = BSP_LSM6DSO_Enqueue(CMD_LSM6DSO_GET_SN, BSP_GET);
		break;
...
static	bool	BSP_LSM6DSO_Transaction(tCmdQueue Rec)
{
	...
	if( result == true)
	{
		...
		if( Main_Set == BSP_GET)
		{
			result = BSP_LSM6DSO_Transaction_TxRx();
		}
		else
		{
			result = BSP_LSM6DSO_Transaction_Tx();
		}
	}
...
{
	Main_Wait4Rx = true;
	return( BSP_LSM6DSO_Transaction_Tx());
}
...
static	bool			BSP_LSM6DSO_Transaction_Tx(void)
{
	tBSP_I2C_TxRx	BSP_I2C_TxRx = {	.handle		= Main_Handle,
										.Address	= I2C_DEVICE_ADDRESS_LSM6DSO,
										.Device		= eBSP_PER_TARGET_LSM6DSO,
										.pData		= Main_TxBuf,
										.Size		= Main_TxLen,
										.Timeout	= (Main_Set == BSP_SET) ? 0 : Main_Timeout,
										.Cb_TxDone	= BSP_LSM6DSO_Cb_TxDone};
...
static	bool			BSP_LSM6DSO_Transaction_Rx(void)
{
	...
	tBSP_I2C_TxRx	BSP_I2C_TxRx = {	.handle		= Main_Handle,
										...
										.Cb_RxDone	= BSP_LSM6DSO_Cb_RxDone};
...
static	void			BSP_LSM6DSO_Cb_RxDone(bool result)
{
	...
	if( BSP_LSM6DSO_Transaction_SetData(Main_Cmd) == true)
	{
		if(Main_Cb_GetData_LSM6DSO != NULL)
			Main_Cb_GetData_LSM6DSO(&Main_Per_DataResp);
	}
}
